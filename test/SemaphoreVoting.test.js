...
describe("SemaphoreVoting", function () {
  ...
  async function deployValidateProofFixture() {
    ...
    const identity2 = new Identity("1")
    const identity3 = new Identity("2")

    const group = new Group()

    group.addMembers([identity.commitment, identity2.commitment, identity3.commitment])

    const proof = await generateProof(identity, group, message, group.root, MERKLE_TREE_DEPTH)
    const proof2 = await generateProof(identity2, group, message, group.root, MERKLE_TREE_DEPTH)
    const proof3 = await generateProof(identity3, group, message, group.root, MERKLE_TREE_DEPTH)



    return { proof, proof2, proof3, group, identity, identity2, identity3 }
  }
  ...
  describe("vote", function () {
    ...
    const nullifier = "14718204733976342144581464809800022797320464223273906549236277227388097861396";
    const voteOption = 0;


    it("Should record a vote correctly", async function () {
      const { proof, identity } = await loadFixture(deployValidateProofFixture)
      await semaphoreVoting.createPoll(
        "Test poll",
    ...
    it("Should allow multiple votes with different nullifiers", async function () {
      const { proof, proof2, identity, identity2 } = await loadFixture(deployValidateProofFixture)
      await semaphoreVoting.createPoll(
        "Test poll",
        ["Option 1", "Option 2", "Option 3"],
        REGISTRATION_DURATION,
      ...
      await semaphoreVoting.registerVoter(0, identity.commitment)
      await semaphoreVoting.registerVoter(0, identity2.commitment)
  ...
  describe("getResults", function () {
    ...
    it("Should return correct results", async function () {
      const { proof, proof2, proof3, identity, identity2, identity3 } = await loadFixture(deployValidateProofFixture)
      await semaphoreVoting.createPoll(
        "Test poll",
        ["A", "B", "C"],
        REGISTRATION_DURATION,
      ...
      await semaphoreVoting.registerVoter(0, identity.commitment);
      await semaphoreVoting.registerVoter(0, identity2.commitment);
      await semaphoreVoting.registerVoter(0, identity3.commitment);
      await time.increase(REGISTRATION_DURATION + 1);

      await semaphoreVoting.vote(0, 0, proof);
      await semaphoreVoting.vote(0, 0, proof2);
      await semaphoreVoting.vote(0, 1, proof3);
    ...
    it("Should return zeros for poll with no votes", async function () {
      const { identity, identity2, identity3 } = await loadFixture(deployValidateProofFixture)
      await semaphoreVoting.createPoll(
        "Test poll",
        ["A", "B", "C"],
        REGISTRATION_DURATION,
